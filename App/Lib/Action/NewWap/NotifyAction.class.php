...
class NotifyAction extends Action {
    ...
    private function chuli($mhtOrderNo, $mhtOrderName, $money) {
        ...
        if (is_array($issc)) {
            ...
            if ($issc[channel_id]) {
                ...
                M('NAgentChannel')->where(array('channel_id' => $issc[channel_id]))->save($data);
            }
            if ($isok && $oks) {
    ...
    private function baoyue($issc) {
        $zhanghu = M('NUserMoney')->where(array('user_id' => $issc[user_id]))->field('vip_time')->find();
        if (strtotime($zhanghu[vip_time]) > strtotime(date('Y-m-d H:i:s', time()))) {
            $newtime = date('Y-m-d H:i:s', strtotime($zhanghu[vip_time] . "+365 day"));
            $oks = M('NUserMoney')->where(array('user_id' => $issc[user_id]))->save(array('vip_time' => $newtime, 'money' => array('exp', "money+$issc[money]")));
            return $oks;
        } else {
